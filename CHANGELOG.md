...
- Fixed the login message reading `Crosshairs vv1.2.3 loaded` — release tags already carry a leading `v`, and both display sites prepended a second one. Running from an unbuilt git clone, where the packager's version keyword is left as-is rather than substituted, now reports as `dev` instead of the raw placeholder.
